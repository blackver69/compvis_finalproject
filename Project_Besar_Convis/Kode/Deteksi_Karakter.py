...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("Knn_Klasifikasi\Data_hasil_klasifikasi\Classifications.txt", np.float32)                  # Membaca data training classifications.txt dari proses pengenalan karakter
    except:
    ...
    try:
        npaFlattenedImages = np.loadtxt("Knn_Klasifikasi\Data_hasil_klasifikasi\Flattened_Images.txt", np.float32)                 # Membaca data training Citra dari proses pengenalan karakter
    except:
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:          # looping untuk setiap kemungkinan plat
        ...
        cv2.imshow("data deteksi",possiblePlate.imgThresh)
